...
router.post('/', (req, res) => {
  Student.create(req.body)
    .then(student => res.json({ msg: ' Registered successfully' }))
...
router.delete('/:id', (req, res) => {
  Student.findByIdAndRemove(req.params.id, req.body)
    .then(student => res.json({ mgs: ' Deleted successfully' }))
    .catch(err => res.status(404).json({ error: 'No Registered Student' }));
});
