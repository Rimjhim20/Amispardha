...
const AllContact = () => {
  ...
  useEffect(async () => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:8082/api/mycontacts`, {
        method: "GET",
        headers: {
  ...
  const deleteContact = async (id) => {
    if (window.confirm("are you sure you want to delete this contact ?")) {
      try {
        const res = await fetch(`http://localhost:8082/api/delete/${id}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
          ...
          <>
            {contacts.length == 0 ? (
              ...
              <>
                ...
                <p>
                   Total Members: <strong>{contacts.length}</strong>
                </p>
